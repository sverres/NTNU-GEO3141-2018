...
printf '%s\n\n' '# GEO3141<br/>Infrastrukturer for stedfestet informasjon<br/>Vår 2018'\
  > "${INDEXFILE}"
...
make_entries "${ASSIGNMENTS}" '## Mappeoppgaver' "${INDEXFILE}"

printf '%s\n' '## Sist oppdatert' >> "${INDEXFILE}"
printf '%s\n' "- $(date +'%F %T') |$(git config --get user.name) |"\
"${COMMITMSG}" >> "${INDEXFILE}"
